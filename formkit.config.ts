...
import { DefaultConfigOptions, defineFormKitConfig } from '@formkit/vue'
import { createProPlugin, inputs } from '@formkit/pro'
import { submit } from '@/FormKit/submit'

const textClassification = {
    input: 'block text-sm font-medium leading-6 text-gray-900 mb-1 block w-full rounded-lg p-3 border-0 focus:shadow-focus ring-1 ring-gray-300 placeholder:text-gray-400 leading-5',
}

const selectClassification = {
    input: `${textClassification.input} !pr-10 form-select`
}

const radioClassification = {
    wrapper: 'flex items-center',
    input: 'h-4 w-4 border-gray-300 text-primary focus:ring-primary',
    label: '!mb-0 ml-3 hover:cursor-pointer'
}
...
export default defineFormKitConfig(() => {
    ...
    const pro = createProPlugin(runtimeConfig.public.FORMKIT_PRO_KEY, inputs)

    const config: DefaultConfigOptions = {
        config: {
            locales: { de },
            locale: 'de',
            // generate more tailwind classes; this may help: https://formkit.com/guides/create-a-tailwind-theme
            classes: generateClasses({
                global: {
                    ...
                    suffixIcon:
                        'w-10 flex self-stretch grow-0 shrink-0 rounded-tr rounded-br border-l border-gray-400 bg-white bg-gradient-to-b from-transparent to-gray-200 [&>svg]:w-full [&>svg]:max-w-[1em] [&>svg]:max-h-[1em] [&>svg]:m-auto',
                },
                ...
                'family:button': {
                    input:
                        '$reset btn btn-primary formkit-disabled:bg-gray-400 formkit-loading:before:w-4 formkit-loading:before:h-4 formkit-loading:before:mr-2 formkit-loading:before:border formkit-loading:before:border-2 formkit-loading:before:border-r-transparent formkit-loading:before:rounded-3xl formkit-loading:before:border-white formkit-loading:before:animate-spin',
                    wrapper: 'mb-1',
    ...
    `,
        ...
        select: {
            ...
            input: 'w-full pl-3 pr-8 py-2 border-none text-base text-gray-700 placeholder-gray-400 fornpm rmkit-multiple:p-0 data-[placeholder="true"]:text-gray-400 formkit-multiple:data-[placeholder="true"]:text-inherit',
        ...
        icons: {
            'check': '<svg width="16" height="16" viewBox="0 0 16 16" fill="#fff" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M5.62812 10.4256L12.2037 3.66675L13.1682 4.58738L5.6503 12.3334L2.83447 9.51755L3.77728 8.57474L5.62812 10.4256Z" fill="#fff"/></svg>',
            'close': '<svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6.91732 1.08331L1.08398 6.91665M1.08398 1.08331L6.91732 6.91665" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/></svg>'
        },
        inputs: {
            // @ts-ignore
            submit
